...
// NOUVEAUX IMPORTS - AgentSaisie et ContestationAmende
import AgentSaisie from './components/AgentSaisie';
import ContestationAmende from './components/ContestationAmende';

// ─── Chatbot Amine flottant (CORRIGÉ) ───────────────────────────────────────
const GlobalChatbot = () => {
        ...
        <div style={chatWrapper} className="slide-up">
          <div style={chatHeader}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              <span style={{ fontSize: '0.9rem', fontWeight: '800' }}>Amine - Assistant Virtuel ASHAL</span>
            </div>
            <X size={20} onClick={() => setIsOpen(false)} style={{ cursor: 'pointer' }} />
          </div>
          <div style={{ flex: 1, overflow: 'hidden', backgroundColor: '#F8FAFC' }}>
            <SupportAide isEmbedded={true} onClose={() => setIsOpen(false)} />
          </div>
        </div>
      ) : (
    ...
    <VaultProvider>
      <Router>
        ...
        <Routes>
          ...
          <Route path="/support-aide" element={<SupportAide isEmbedded={true} />} />
          
          {/* NOUVELLES ROUTES - AgentSaisie et ContestationAmende */}
          <Route path="/agent-saisie" element={<AgentSaisie />} />
          <Route path="/contestation-amende" element={<ContestationAmende />} />
        </Routes>
