...
import { 
  ...
  Lock, Wallet, Landmark, Loader2, 
  Signal, Wifi, Battery, Nfc, Cpu
} from 'lucide-react';
import jsPDF from 'jspdf';
import { documentsAPI, getCin } from '../services/api';
...
const handlePayment = async () => {
    // 1. Validation basique du formulaire
    ...
    try {
      // 2. Récupération du CIN réel depuis le localStorage via le helper
      const cin = getCin(); 

      // 3. Appel de l'API avec les bonnes clés attendues par votre backend
      const demande = await documentsAPI.creerDemandeDocument({
        cinDemandeur: cin, // Utilise le vrai CIN au lieu d'un texte fixe
      ...
      // 4. Succès : stockage de la référence et passage à l'étape de confirmation
      setReference(demande.reference);
      setStep(4);
    } catch (err) {
      // 5. Gestion des erreurs (ex: "Data too long", erreur serveur, etc.)
  ...
  const handleDownload = () => {
    ...
    doc.setFontSize(18);
    doc.text('EXTRAIT DE NAISSANCE NUMERIQUE', 105, 55, { align: 'center' });
    ...
    doc.setFont('helvetica', 'bold');
    doc.text('DETAILS DU TITULAIRE', 105, 100, { align: 'center' });
    doc.setFont('helvetica', 'normal');
    doc.text(`Nom et Prenom : ${cardHolder.toUpperCase()}`, 30, 115);
    doc.text(`Nationalite : Tunisienne`, 30, 125);
    doc.text(`Statut : Document Officiel Certifie`, 30, 135);

    doc.setFontSize(10);
    doc.text(`Montant paye : 0.600 DT`, 20, 160);
    doc.text(`Transaction securisee via : ${paymentType === 'bank' ? 'Carte Bancaire' : 'E-Dinar'}`, 20, 167);

    doc.rect(150, 220, 30, 30);
    doc.setFontSize(7);
    doc.text('QR CODE DE VERIFICATION', 147, 255);
    doc.text('SIGNATURE ELECTRONIQUE VALIDE', 145, 215);
            ...
            <div className="slide-up">
              ...
              <div style={formBox}>
                ...
                <button onClick={handlePayment} disabled={loading} style={payBtn}>
                  {loading ? <Loader2 className="spin" size={20} /> : 'PAYER 0.600 DT'}
                </button>
              </div>
            </div>
          )}
