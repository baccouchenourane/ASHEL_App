...
import {
  Star, Send, ChevronLeft, CheckCircle, AlertCircle,
  Signal, Wifi, Battery,
...
const Evaluation = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const cin = user.cin || '12345678';
      
      const response = await fetch('/api/evaluations', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ...form, citoyenId: cin }),
      });
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div style={{ marginBottom: '18px', display: 'flex', gap: '10px' }}>
                {[1, 2, 3, 4, 5].map((star) => (
                  <span
                    key={star}
                    onClick={() => { setError(''); setForm({ ...form, note: star }); }}
                    onMouseEnter={() => setHovered(star)}
                    onMouseLeave={() => setHovered(0)}
                    style={{
                      fontSize: '2.5rem',
                      cursor: 'pointer',
                      color: star <= (hovered || form.note) ? '#E70011' : '#E2E8F0',
