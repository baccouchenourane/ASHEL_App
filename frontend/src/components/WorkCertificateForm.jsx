...
import { 
  ...
  Lock, Wallet, FileText, Download, Loader2, 
...
const WorkCertificateForm = ({ onBack }) => {
  ...
  const handleDownload = () => {
    ...
    doc.text(`Fait à Tunis, le ${dateStr}`, 140, 60);

    doc.setFontSize(12);
    const texte = `Je soussigné, Responsable des Ressources Humaines, certifie par la présente que M/Mme ${cardHolder.toUpperCase()} est employé(e) au sein de notre établissement depuis le 01/01/2020 en qualité de Cadre.`;
    
    const splitTitle = doc.splitTextToSize(texte, 160);
    doc.text(splitTitle, 25, 90);

    doc.text("Cette attestation est délivrée à l'intéressé(e) pour servir et valoir ce que de droit.", 25, 130);

    // Signature
    doc.setFont("helvetica", "bold");
    doc.text("La Direction Générale", 140, 180);
    doc.rect(145, 185, 30, 20); // Emplacement cachet
            ...
            <div className="slide-up">
              <div style={cardPreview(getCardType(cardNumber))}>
                ...
                <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end'}}>
                    <div><p style={cardLabel}>SALARIÉ</p><p style={cardVal}>{cardHolder.toUpperCase() || "NOM DU COLLABORATEUR"}</p></div>
                    <img src={getCardType(cardNumber) === 'visa' ? "https://upload.wikimedia.org/wikipedia/commons/5/5e/Visa_Inc._logo.svg" : "https://upload.wikimedia.org/wikipedia/commons/2/2a/Mastercard-logo.svg"} style={{height: '25px', opacity: getCardType(cardNumber) === 'default' ? 0 : 1}} />
                </div>
              </div>
              ...
              <div style={formBox}>
                <div style={inputGroup}>
                    ...
                    <input type="text" placeholder="0000 0000 0000 0000" style={fInput} value={cardNumber} onChange={(e) => setCardNumber(e.target.value.replace(/\D/g, '').replace(/(.{4})/g, '$1 ').trim())} maxLength={19} />
                </div>
                <div style={inputGroup}>
                ...
                <button onClick={handlePayment} disabled={loading} style={payBtn}>{loading ? <Loader2 className="spin" /> : `PAYER 1.500 DT`}</button>
              </div>
            </div>
          )}
            ...
            <div style={{textAlign: 'center'}} className="scale-in">
                ...
                <div style={receipt}>
                    <div style={receiptTop}>
                        ...
                        <p style={{fontSize: '11px', color: '#64748b'}}>RH-CERT-2026</p>
                    </div>
