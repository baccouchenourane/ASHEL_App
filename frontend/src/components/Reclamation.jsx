...
import {
  FileText, Send, ChevronLeft, Signal, Wifi, Battery,
  CheckCircle, AlertCircle, LayoutGrid, CreditCard, Users, User
} from 'lucide-react';
import logoAshel from '../assets/logo_ashel.png';

const SERVICES = ['Mairie', 'Hôpital', 'Transport en commun', 'École publique', 'Poste', 'Autre'];

const Reclamation = () => {
  const navigate = useNavigate();
  const [form, setForm] = useState({ objet: '', contenu: '' });
  const [objetCustom, setObjetCustom] = useState('');
  const [error, setError] = useState('');
  const [success, setSuccess] = useState(false);
  const [loading, setLoading] = useState(false);

  // Value actually sent to the API
  const objetFinal = form.objet === 'Autre' ? objetCustom.trim() : form.objet;
  ...
  const handleSubmit = async (e) => {
    ...
    if (form.objet === 'Autre' && !objetCustom.trim()) {
      setError('Veuillez préciser le service concerné.');
      return;
    }
    ...
    <div style={S.screenWrapper}>
      <div className="app-container">
        ...
        <div style={S.hero}>
          ...
          <div style={S.heroBody}>
            ...
            <h2 style={S.heroTitle}>Réclamation</h2>
            <p style={S.heroSub}>Déposez une réclamation officielle</p>
          </div>
        </div>
        ...
        <div style={S.scrollArea}>
          ...
          {success && (
            <div style={S.successBanner} className="fade-in">
              <CheckCircle size={16} />
              <span>Réclamation déposée. Vous serez contacté sous 48h.</span>
            </div>
          )}
          ...
          <div style={S.card} className="fade-in">
            <form onSubmit={handleSubmit}>
              ...
              <p style={S.fieldLabel}>SERVICE CONCERNÉ</p>
              ...
              {/* Custom service input — shown only when "Autre" is selected */}
              {form.objet === 'Autre' && (
                <div style={{ ...S.inputWrap, marginTop: '-10px' }} className="fade-in">
                  <input
                    type="text"
                    placeholder="Précisez le service concerné..."
                    value={objetCustom}
                    onChange={e => { setError(''); setObjetCustom(e.target.value); }}
                    required
                    autoFocus
                    style={{
                      ...S.input,
      ...
      <style>{`
        ...
        *::-webkit-scrollbar { display: none; }
        ...
        input:focus, textarea:focus, select:focus { border-color: #0056D2 !important; outline: none; background: white !important; }
      `}</style>
    </div>
  );
};
